...
import { 
    Form, FormControl, Label, Box,
    Select, Option, Card, Button, HelpText,
    Modal, ModalHeader, ModalHeading, ModalBody, Paragraph,
    Grid, Column
...
const transcriptionProviders = [{name: "Google", value: 'Google'}, {name: "Deepgram", value: 'Deepgram'}];
const ttsProviders = [{name: "Google", value: 'Google'}, {name: 'Amazon', value: 'Amazon'}, {name: 'Eleven Labs', value: 'ElevenLabs'}];


const UseCaseCombo = (props) => { 
    // local component state
    const [useCases, setUseCases] = useState(null);
    const [selectedUseCase, setSelectedUseCase] = useState(props.useCases[0] || null);
    ...
    const [showVoiceDialog, setShowVoiceDialog] = useState(false);
      ...
      useEffect(() => { 
        ...
        setSelectedSttProvider(props.selectedUser?.conversationRelayParamsOverride?.transcriptionProvider)
        setSelectedTtsProvider(props.selectedUser?.conversationRelayParamsOverride?.ttsProvider)
        setSelectedVoice(props.selectedUser?.conversationRelayParamsOverride?.voice)
        setSpeechModel(props.selectedUser?.conversationRelayParamsOverride?.speechModel)
      ...
      useEffect(() => {
        ...
        const getVoices = async () => {
            ...
            try {
                // Fetch all voices from the backend 
                await axios.get(voicesURL)
                .then((resp) => {
                    setVoices(resp.data);
                    // set the selected voiced based on the user settings
                    const providerVoices = resp.data.find((provider) => provider.key === props.selectedUser?.conversationRelayParamsOverride?.ttsProvider)
      ...
      const useCaseChange = (e) => {
        ...
        if (selectedItem) {
            ...
            selectedItem.value.conversationRelayParams.transcriptionProvider==='Google' ? 
    ...
    let layout = (
        <Form>
            <FormControl>
                <Label htmlFor="useCaseSelect" required>Select a Use Case</Label>
                <Select
                    id="useCaseSelect"
                ...
                >
                    {
                        useCases?.map((useCase) => {
                            return (
                                <Option key={useCase.key} value={useCase.key}>{useCase.key}</Option>
                            )
                        })  
                    }
                </Select>
                <Box as="div" marginTop="space40">
                    <Card>
                        ...
                        <Box as={'div'} >
                        <Label htmlFor="stt_provider" required>Transcription (STT) Provider</Label>
                        <Select
                            id="stt_provider"
                        ...
                        <Box as={'div'} marginTop="20px">
                            <Label htmlFor="tts_provider" required>Text-to-Speech (TTS) Provider</Label>
                            <Select
                                id="tts_provider"
                        ...
                        <Box as={'div'} marginTop="20px">
                            <Label htmlFor="voice" required>Voice</Label>
                            <Select
                                id="voice"
                                htmlFor="voice"
                                value={selectedVoice}
            ...
            <Modal ariaLabelledby={'voiceDialog'} isOpen={showVoiceDialog} onDismiss={(e) =>handleCloseVoiceDialog(e)} size="default">
                <ModalHeader>
                ...
                <Paragraph>
                    Conversation Relay supports a wide variety of TTS voices (2000+) from several providers (Google, Amazon and Eleven Labs).
                </Paragraph>
                <Paragraph>This demo highlights only a small subset of the available voices.</Paragraph>
                <Paragraph>
                    For complete details on supported providers and genders, languages and voice support, please refer to the product documentation.
                </Paragraph>
                <Grid gutter="space40" marginTop="space40">
                    <Column span={6}>
                        ...
                        <Box as="div" style={{textAlign:'center'}}>
                            ...
                            target="_blank">Google and Amazon Voices 
                            </Box>
                        </Box>
                    </Column>
                    <Column span={6}>
                        <Box as="div" style={{textAlign:'center'}}>                   
                            <Box as="a" 
                                ...
                                target="_blank">Eleven Labs Voices
                            </Box>
                        </Box>
                    </Column>
                    </Grid>
